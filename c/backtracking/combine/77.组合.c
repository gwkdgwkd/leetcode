...
// 回溯算法
// 回溯法也可以叫做回溯搜索法，它是⼀种搜索的⽅式。回溯是递归的副产品，只要有递归就会有回溯。回溯函数也就是递归函数，指的都是⼀个函数。
// 因为回溯的本质是穷举，穷举所有可能，然后选出我们想要的答案，如果想让回溯法⾼效⼀些，可以加⼀些剪枝的操作，但也改不了回溯法就是穷举的本质。
// 回溯法解决的问题都可以抽象为树形结构，回溯法解决的都是在集合中递归查找⼦集，集合的⼤⼩就构成了树的宽度，递归的深度，都构成的树的深度。
// 递归就要有终⽌条件，所以必然是⼀颗⾼度有限的树（N叉树）。
// 回溯法解决的问题都可以抽象为树形结构（N叉树），⽤树形结构来理解回溯就容易多了。
// 回溯是递归的副产品，只要有递归就会有回溯，所以回溯法也经常和⼆叉树遍历，深度优先搜索混在⼀起，因为这两种⽅式都是⽤了递归。
