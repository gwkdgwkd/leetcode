...
每个字符串方程equations[i]的长度为4，
并采用两种不同的形式之一："a==b"或"a!=b"。
在这里，a和b是小写字母（不一定不同），表示单字母变量名。
只有当可以将整数分配给变量名，
以便满足所有给定的方程时才返回true，否则返回false。
...
// 连通两个节点Union、判断两个节点的连通性Connected、
// 计算连通分量Count所需的时间复杂度均为O(1)。
int count;    // 连通分量个数
int* parent;  // 存储一棵树
int* size;    // 记录树的重量
void UF(int n) {
...
int Find(int x) {
  // 主要API
  // Connected和Union中的复杂度都是Find函数造成的，
  // 所以说它们的复杂度和Find一样。
  // 能不能进一步压缩每棵树的高度，使树高始终保持为常数？
  // 这样Find就能以O(1)的时间找到某一节点的根节点，相应的，
  // Connected和Union复杂度都下降为O(1)。
  // 调用Find函数每次向树根遍历的同时，顺手将树高缩短了，
  // 最终所有树高都不会超过3（Union的时候树高可能达到3）
  while (parent[x] != x) {
    // 进行路径压缩
...
void Union(int p, int q) {
  ...
  // 通过比较树的重量，就可以保证树的生长相对平衡，
  // 树的高度大致在logN这个数量级，极大提升执行效率。
  // 此时，Find,Union,Connected的时间复杂度都下降为O(logN)，
  // 即便数据规模上亿，所需时间也非常少。
