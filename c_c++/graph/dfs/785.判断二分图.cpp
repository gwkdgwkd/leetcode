...
// 随后对整个图进行遍历，将该节点直接相连的所有节点染成绿色，
// 表示这些节点不能与起始节点属于同一个集合。
// 再将这些绿色节点直接相连的所有节点染成红色，以此类推，直到无向图中的每个节点均被染色。
// 如果能够成功染色，那么红色和绿色的节点各属于一个集合，这个无向图就是一个二分图；
// 如果未能成功染色，即在染色的过程中，某一时刻访问到了一个已经染色的节点，
// 并且它的颜色与我们将要给它染上的颜色不相同，也就说明这个无向图不是一个二分图。
// 算法的流程如下：
// 1.任选一个节点开始，将其染成红色，并从该节点开始对整个无向图进行遍历；
// 2.在遍历的过程中，如果通过u遍历到了v，即u和v在图中有一条边直接相连，那么会有两种情况：
//   a.如果v未被染色，那么我们将其染成与u不同的颜色，并对v直接相连的节点进行遍历；
//   b.如果v被染色，并且颜色与u相同，那么说明给定的无向图不是二分图，退出返回false。
