...
class Solution {
 public:
  ...
  int longestIncreasingPath(vector<vector<int> >& matrix) {
    ...
    auto outdegrees = vector<vector<int>>(m, vector<int>(n));
    for (int i = 0; i < m; ++i) {
      for (int j = 0; j < n; ++j) {
        for (int k = 0; k < 4; ++k) {
          int newRow = i + dirs[k][0], newColumn = j + dirs[k][1];
