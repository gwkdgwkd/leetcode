...
// 当然，这是理论上的最坏复杂度，
// 实际的复杂度肯定要好一些，毕竟我们添加了这么多剪枝逻辑。
// 不过，从复杂度的上界已经可以看出第一种思路要慢很多了。
// 通俗来说，我们应该尽量少量多次，
// 就是说宁可多做几次选择，也不要给太大的选择空间；
// 宁可二选一选k次，也不要k选一选一次。
bool *used;
...
class Solution {
 public:
  bool backTracking(vector<int> &nums, vector<bool> &used, int k, int sum,
    ...
    if (sum == target) {
      // 此次划分等于目标值
      return backTracking(nums, used, k - 1, 0, target, 0);
    }
...
class Solution {
 public:
  bool dfs(vector<int> &nums, int k, int target, vector<int> &bucket,
           int index) {
    if (index == nums.size()) {      // 所有元素的枚举完了
    ...
    for (int i = 0; i < k; ++i) {              // 枚举每一个桶
      if (bucket[i] + nums[index] > target) {  // 当前桶装不下了
        continue;                              // 剪枝
      }
      bucket[i] += nums[index];  // 装了
      if (dfs(nums, k, target, bucket, index + 1)) {
        return true;  // 判断当前桶装了之后，下一个是否还能正常装好
      }
      bucket[i] -= nums[index];  // 下一个不能，说明这个球不是这个桶的，回溯
  ...
  bool canPartitionKSubsets(vector<int> &nums, int k) {
    int sum = accumulate(nums.begin(), nums.end(), 0);  // 计算每个子集的和
    if (sum % k != 0) {
      return false;
    }
    int target = sum / k;
    vector<int> b(k, 0);
    return dfs(nums, k, target, b, 0);
  }
};