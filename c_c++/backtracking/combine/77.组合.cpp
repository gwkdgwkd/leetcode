...
// ⽤树形结构来理解回溯就容易多了。
// 回溯是递归的副产品，只要有递归就会有回溯，
// 所以回溯法也经常和⼆叉树遍历，深度优先搜索混在⼀起，
// 因为这两种⽅式都是⽤了递归。

// 回溯三部曲:
// 1.回溯函数模板返回值以及参数，回溯算法中函数返回值⼀般为void。
//   void backtracking(参数)
// 2.回溯函数终⽌条件，⼀般来说搜到叶⼦节点了，
//   也就找到了满⾜条件的⼀条答案，把这个答案存放起来，并结束本层递归。
//   if (终⽌条件) {
//    存放结果;
//    return;
//   }
// 3.回溯搜索的遍历过程，回溯法⼀般是在集合中递归搜索，
...
// 定义两个全局变量，⼀个⽤来存放符合条件单⼀结果，
// ⼀个⽤来存放符合条件结果的集合。
// 其实不定义这两个全局遍历也是可以的，
// 把这两个变量放进递归函数的参数⾥，但函数⾥参数太多影响可读性。
int* path;
int pathSize;
int** result;
int resultSize;
// 每次从集合中选取元素，可选择的范围随着选择的进⾏⽽收缩，
...
void backtracking(int n, int k, int startIndex) {
  ...
  // 如果for循环选择的起始位置之后的元素个数已经不满足需要的元素个数了，
  // 那么就没有必要搜索了。
