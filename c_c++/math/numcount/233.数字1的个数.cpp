...
// 思路：
// 枚举每一个数位，分别统计该数位上数字1出现的次数，最后进行累加即可得到答案。
// 以n=1234567为例，统计百位上数字1出现的次数：
// 1.对于从0开始每1000个数，百位上的数字1都会出现100次，呈现[000,999]的循环，
//   其中的[100,199]在百位上的数字为1，共有100个。
//   n拥有1234个这样的循环，每个循环百位上都出现了100次1，一共出现了1234×100次1。
//   公式表示：(n/1000)*100
// 2.对于剩余不在完整的循环中的部分，最后三位为[000,567]，
//   其中567可以用mod=n%1000表示，这部分的1可以分为三类：
//   a.当mod<100时，百位上不会出现1；
//   b.当100<=mod<200时，百位上出现1的范围为[100,mod]，所以出现了mod-100+1次1；
