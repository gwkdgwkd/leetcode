/*
给一个字符串s，请统计并返回这个字符串中回文子串的数目。
回文字符串是正着读和倒过来读一样的字符串，子字符串是由连续字符组成的一个序列。
...
int countSubstrings(char* s) {
  ...
  // 根据不确定是不是回⽂的区间[i+1,j-1]来判断了[i,j]是不是回⽂，结果错误。
  // 所以⼀定要从下到上，从左到右遍历，这样保证dp[i+1][j-1]都是经过计算的。
  for (int i = n - 1; i >= 0; --i) {
    for (int j = i; j < n; ++j) {
      // 递推公式整体上就是s[i]与s[j]相等，s[i]与s[j]不相等这两种：
      ...
      //      那么就看区间i+1到j-1是不是回⽂，也就是看dp[i+1][j-1]是否为true。
      if (s[i] == s[j]) {
        if (j - i <= 1) {  // 情况一和情况二
          ++result;
          dp[i][j] = true;
