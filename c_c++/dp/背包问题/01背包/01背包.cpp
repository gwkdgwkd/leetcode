...
int test1(int *weight, int *value, int size, int bagweight) {
  // 1.确定dp数组以及下标的含义：
  //   dp[i][j]表示从0到i的物品里任意取，放进容量为j的背包，价值和最大是多少；
  ...
  //     dp[0][j] = 0;
  //   }
  //   当j>=weight[0]时，dp[0][j]应该是value[0]，因为背包可以放下weight[0]的物品：
  for (int j = weight[0]; j <= bagweight; ++j) {
    dp[0][j] = value[0];
  }
  //   dp其他值初始化什么数值都可以，因为都会被覆盖，所以可以一开始全都初始化成0更方便些。
  ...
  for (int i = 1; i < size; ++i) {          // 遍历物品
    for (int j = 0; j <= bagweight; ++j) {  // 遍历背包容量
      // 2.确定递推公式
      if (j < weight[i]) {
        // 背包容量为j，里面放不下物品i，此时最大价值dp[i][j]等于dp[i-1][j]：
        dp[i][j] = dp[i - 1][j];
      } else {
        // dp[i-1][j]：背包容量为j的时候不放物品i的最大价值；
...
int test2(int *weight, int *value, int size, int bagweight) {
  ...
  // 与其把dp[i-1]这⼀层拷⻉到dp[i]上，不如只⽤⼀维数组dp[j]，这是⼀个滚动数组。
  // 需要满⾜的条件是上⼀层可以重复利⽤，直接拷⻉到当前层。
  // 1.确定dp数组的定义：
  //   在⼀维dp数组中，dp[j]表示容量为j的背包，所装的物品价值可以最⼤为dp[j]。
  int dp[bagweight + 1];
  // 3.⼀维dp数组如何初始化：
