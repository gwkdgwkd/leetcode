...
int minDistance(char* word1, char* word2) {
  ...
  dp[0][0] = 0;
  for (int i = 1; i <= m; ++i) {
    // dp[i][0]：以下标i-1为结尾的字符串word1，
    // 和空字符串word2，最近编辑距离为dp[i][0]。
    // 那么dp[i][0]就应该是i，对word1⾥的元素全部做删除操作，
    // 即：dp[i][0]=i;
    dp[i][0] = i;
  }
  for (int j = 1; j <= n; ++j) {
  ...
  for (int i = 1; i <= m; ++i) {
    for (int j = 1; j <= n; ++j) {
      if (word1[i - 1] == word2[j - 1]) {
        ...
        //   那么就是以下标i-2为结尾的word1与i-1为结尾的word2的最
        //   近编辑距离加上⼀个增加元素的操作。
        //   即dp[i][j] = dp[i-1][j] + 1;
        // 2.word2添加⼀个元素，使其word1[i-1]与word2[j-1]相同，
        //   那么就是以下标i-1为结尾的word1与j-2为结尾的word2的最
        //   近编辑距离加上⼀个增加元素的操作。
        //   即dp[i][j]=dp[i][j-1] + 1;
        //   只有添加元素，删除元素去哪了。
        //   word2添加⼀个元素，相当于word1删除⼀个元素。
        // 3.替换元素，word1替换word1[i-1]，使其与word2[j-1]相同，
        //   此时不⽤增加元素，那么以下标i-2为结尾的word1与j-2为结尾
        //   的word2的最近编辑距离加上⼀个替换元素的操作。
