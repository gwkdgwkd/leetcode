...
其他房间要么是空的（房间里的值为0），要么包含增加骑士健康点数的魔法球，
若房间里的值为正整数，则表示骑士将增加健康点数。
为了尽快到达公主，骑士决定每次只向右或向下移动一步。

编写一个函数来计算确保骑士能够拯救到公主所需的最低初始健康点数。
例如，考虑到如下布局的地下城，如果骑士遵循最佳路径右->右->下->下，则骑士的初始健康点数至少为7。
-2(K) -3 	  3
-5 	  -10 	1
10 	  30 	  -5(P)
...
int calculateMinimumHP(int** dungeon, int dungeonSize, int* dungeonColSize) {
  ...
  int dp[n + 1][m + 1];  // p[i][j]表示从坐标(i,j)到终点所需的最小初始值
  ...
  for (int i = n - 1; i >= 0; --i) {
    for (int j = m - 1; j >= 0; --j) {
      ...
      dp[i][j] = fmax(minn - dungeon[i][j], 1);  // 初始值还必须大于等于1
    }
  }
