/*
在一个nxn的矩阵grid中，
除了在数组mines中给出的元素为0，其他每个元素都为1。
mines[i] = [xi, yi]表示grid[xi][yi] == 0，
返回grid中包含1的最大的轴对齐加号标志的阶数。
如果未找到加号标志，则返回0。
一个k阶由1组成的轴对称加号标志具有中心网格grid[r][c] == 1，
...
class Solution {
 public:
  int orderOfLargestPlusSign(int n, vector<vector<int>>& mines) {
    ...
    for (int i = 1; i <= n; i++) {
      for (int j = 1; j <= n; j++) {
        arms[i][j][0] = (graph[i][j] == 0 ? 0 : arms[i - 1][j][0] + 1);
        arms[i][j][2] = (graph[i][j] == 0 ? 0 : arms[i][j - 1][2] + 1);
      }
    }
    // 计算下、右臂
    for (int i = n; i >= 1; i--) {
      for (int j = n; j >= 1; j--) {
        arms[i][j][1] = (graph[i][j] == 0 ? 0 : arms[i + 1][j][1] + 1);
        arms[i][j][3] = (graph[i][j] == 0 ? 0 : arms[i][j + 1][3] + 1);
      }
    }
    int ans = 0;
    for (int i = 1; i <= n; i++) {
