...
// 确定当前的鸡蛋个数和面对的楼层数，
// 就知道最小扔鸡蛋次数，最终要的答案就是dp(K,N)的结果。
// 这种思路下，肯定要穷举所有可能的扔法，
// 用下面的二分搜索优化也只是做了减枝，减小了搜索空间，
// 但本质思路没有变，还是穷举。
...
// 修改dp数组的定义，确定当前的鸡蛋个数和最多允许的扔蛋次数，
// 就知道能够确定F的最高楼层数。
// 时间复杂度：O(KN)
// dp转移只和左边和左上的两个状态有关，所以很容易优化成一维dp数组，
// 还可以借助二分搜索法快速逼近dp[K][m]==N这个终止条件，
// 时间复杂度进一步下降为O(KlogN)。
int superEggDrop(int k, int n) {
  // dp数组的含义：
  // dp[k][m]=n表示有k个鸡蛋，扔m次，最坏情况下最多可以扔n层楼
  ...
  while (dp[k][m] < n) {  // dp[k][m] == n表示找到最多次数m
    ...
    for (int i = 1; i <= k; ++i) {
      ...
      // dp[k][m-1]就是楼上的楼层数，因为鸡蛋个数k不变，
      // 也就是鸡蛋没碎，扔鸡蛋次数m减一；
      // dp[k-1][m-1]就是楼下的楼层数，因为鸡蛋个数k减一，
      // 也就是鸡蛋碎了，同时扔鸡蛋次数m减一。
