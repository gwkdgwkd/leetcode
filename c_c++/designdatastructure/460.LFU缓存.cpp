...
// 同时删除key_table中对应的索引，最后向key_table和S插入新的缓存信息即可。
class LFUCache {
  // 缓存容量，时间戳
  int capacity, time;
...
class LFUCache {
 ...
 public:
  ...
  // 对于put(key, value)操作，先通过索引key在 key_table中查看是否有对应的缓存，
  // 如果有的话，等价于get(key)操作，唯一的区别就是需要将当前的缓存里的值更新为value。
  ...
  // 所以freq_table[minFreq]的链表中链表尾的节点，即为使用频率最小且插入时间最早的节点，
