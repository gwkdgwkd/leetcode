...
在处理标点符号和大小写之前，得先把它断成词语。
有词典dictionary，不过，有些词没在词典里，假设文章用sentence表示，
...
int respace(char** dictionary, int dictionarySize, char* sentence) {
  ...
  int dp[n + 1];  // dp[i]表示字符串的前i个字符的最少未匹配数
  memset(dp, 0x3f, sizeof(dp));
  dp[0] = 0;
  for (int i = 1; i <= n; ++i) {
    // 假设当前已经考虑完了前i个字符了，对于前i+1个字符对应的最少未匹配数：
    // 第i+1个字符未匹配，则dp[i+1]=dp[i]+1，即不匹配数加1；
    // 遍历前i个字符，若以下标j为开头、以第i+1个字符为结尾的字符串正好在词典里，
    // 则dp[i] = min(dp[i], dp[j])更新dp[i]。
    dp[i] = dp[i - 1] + 1;
